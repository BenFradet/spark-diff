...
/** Represents intermediate delta information for one pair of rows.
  * @param keys
  *   the values for the composite primary key
  * @param deltas
  *   the deltas produced by the comparison
  * @param diffType
  *   difference type for the two rows: whether they are the same, different, missing their left-hand side or right-hand
  *   side or incomparable
  */
...
object DeltaValue {
  ...
  /** Represents a value for which we can't compute the delta, this is the case for nested types for example.
    * @param sparkType
  ...
  /** Represents a computed delta.
    * @param deltaType
    *   type of the delta: either numeric or string-based
    * @param value
    *   of the detla between two values
    */
  ...
  object NumericDelta {
    ...
    /** Creates a numeric delta from a delta value.
      * @param value
      *   delta value computed using substraction
      * @return
      *   a numeric delta
      */
  ...
  object StringDelta {
    ...
    /** Creates a string-based delta from a delta value.
      * @param value
      *   delta value computed using the Levenshtein distance
      * @return
      *   a numeric delta
      */
...
/** Represents a delta between two values.
  * @param fieldName
  *   name of the field from the two datasets we are comparing
  * @param left
  *   value
  * @param right
  *   value
  * @param deltaValue
  *   computed delta between left and right
  */
...
object Delta {
  ...
  /** Computes the deltas between two Spark rows, field by field. If the schema of the two rows is not the same a left
    ...
    * numeric types produce a numeric delta. Dates, timestamps and strings produce a string-based delta. Other types
    * (notably nested types) are not supported.
    * @param l
    *   left row
    * @param r
    *   right row
    * @return
    *   either the list of computed deltas or an error if the schema of the two rows is not the same
    */
    ...
    if (l.schema =!= r.schema) {
      ...
      l.schema.fields.toList
        .map { case StructField(name, dt, _, _) =>
          ...
          dt match {
            case ByteType | DoubleType | FloatType | IntegerType | LongType | ShortType | _: DecimalType =>
  ...
  /** Extracts a double value from an Any. It doesn't check that the value is actually numeric, this should be checked
    * beforehand.
    * @param a
    *   an Any
    * @return
  ...
  /** Extracts a double value from an Any representing a Boolean. It doesn't check that the value is actually a boolean,
    * this should be checked beforehand.
    * @param a
    *   an Any
    * @return
    *   a double, if the boolean is false returns 0, otherwise returns 1
    */
  ...
  /** Extracts a value from a row through its field name.
    ...
    *   row
    * @param fieldName
    *   name of the field to extract the value with
    * @return
    *   the value if the field name exists in the row and a value can be extracted, None if the value is null.
    */
  ...
  /** Computes a delta between two options:
    ...
    *   right value
    * @param f
    *   turns one value into a double
    * @param g
    *   turns two values into a double
    * @return
    *   a delta
    */
